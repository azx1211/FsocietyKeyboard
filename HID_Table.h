...
// HID Table code. According to HID standard
// http://www.freebsddiary.org/APC/usb_hid_usages.php under section 7
...
// Consumer Report shortcut
//#define KEY_CRHOME	

// OWN Application CODE here
#define GOTOCOMMANDMODE			0xE8
...
#define MODIFIER_FN		  77

#define MOUSDE_LEFT_CLICK		(1<<0)
#define MOUSDE_RIGHT_CLICK		(1<<1)


char HIDtoPrintableASCII(byte in){
	
	if(in<=KEY_0){ // Scan only the printable character. limit until key 0. see table above
		if(in<=KEY_Z){ // limit the characters
			return in+61; // Add with 61 which is the diference between HID and ASCII
		}else if(in<KEY_0){ // limit the numbers/ 
			return in+19; // add with 19 which is the interval for numbers between HID and ASCII
		}else if (in==KEY_0){
			return 48; // LAST one, special case. check if it is 0 character. 
		}
	}
	return NULL;
}